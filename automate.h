...
struct transition {
    char car; // caractère étiquetant la transition.
              // -1 si c'est une epsilon transition
...
typedef struct{
    int sigma; // Le nombre de lettre de l'alphabet, les lettres de l'alphabet
               // étant les minuscules. Ce qui limite donc à 26

    int Q;     // Le nombre d'états qui sont numérotés de 0 à Q-1.
               // 0 étant l'état initial

    int *F;    // Un tableau monodimensionnel de taille Q qui indique pour
               // chaque état s'il est final ou non

    TRANSITION *T; // Un tableau de taille Q donnant pour chaque état la
...
// Copie une liste de transitions, en ajoutant 'décalage' au numero de
// l'état d'arrivée
...
// Créé une copie fraiche de l'automate A
// Si conserve_epsilon == 0, ne conserve pas les epsilon transitions
AUTOMATE automate_copier(AUTOMATE A, int conserve_epsilon);

// Ajoute la transition (dep,car,arr) à l'automate (ne fait
// rien si elle existe)
...
// Ajoute l'état q aux états finaux
void automate_ajouter_final (AUTOMATE A, int q);

// Supprime l'état q des états finaux
// void automate_supprimer_final (AUTOMATE A, int q);

// Libère la mémoire allouée dynamiquement pour l'automate
void automate_liberer_memoire(AUTOMATE A);

// Ecrit un automate dans un fichier sous la forme :
/*
Q F T
F_1 F_2 F_3 ... F_F
dep_1 car_1 arr_1
...
// Renvoie un automate sans epsilon transition qui
// reconnait le même langage que A
AUTOMATE automate_supprimer_epsilon(AUTOMATE A);

//////////////////////////// Fonction à programmer vous même /////////////////////////////////////////////////

// Renvoie l'automate qui reconnait le mot epsilon
AUTOMATE automate_epsilon();

// Renvoie l'automate qui reconnait le mot
// constitué de la lettre car ou epsilon si car = -1
AUTOMATE automate_une_lettre(char car);

// Renvoie un automate qui reconnaît la concaténation des
// langages reconnus par A et B
AUTOMATE automate_concatenation(AUTOMATE A, AUTOMATE B);

// Renvoie un automate qui reconnaît la disjonction des
// langages reconnus par A et B
AUTOMATE automate_disjonction(AUTOMATE A, AUTOMATE B);

// Renvoie un automate qui reconnaît
// l'étoile de Kleene du langage du langage reconnu par A
AUTOMATE automate_etoile(AUTOMATE A);

// Renvoie un automate déterministe qui reconnait le même langage que A
